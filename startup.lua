...
function websocketLoop()
	local req,cErr = http.get("http://raw.githubusercontent.com/TheQWERTYCodr/qwertyTurtleCtrl/main/server")
	local server = ""
	if req then
		server = req.readAll()
		req.close()
		local file = fs.open("server","w")
		file.write(server)
		file.close()
	else
		print("Server URI check failed. Reason:")
		print(cErr)
		if fs.exists("server") then
			print("Attempting connection with last known server...")
			local file = fs.open("server","r")
			server=file.readAll()
			file.close()
		else
			print("No fallback available, rebooting...")
			os.sleep(3)
			os.reboot()
		end
	end
	...
	elseif ws then
		while true do
			...
			print("{O}+TheQWERTYCoder\n\nActive connection established, do not interrupt.")
...
while true do
	...
	print("{O}+TheQWERTYCoder\n\nSleep mode active, please don't break me!")
	os.sleep(5)
end
